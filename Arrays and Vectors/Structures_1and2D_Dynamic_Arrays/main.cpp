 ...
 * Created on September 29, 2021, 12:55 PM
 * Purpose:  Structures with 1 and 2 Dimensional Dynamic Arrays
 */
...
int main(int argc, char** argv) {
    ...
    fillAry(table,array,0);         //Fills table columns with data array
...
void destroy(Array1D *a){
    ...
    delete a;         //Deallocate structure
}

void destroy(Array2D *a){
    for(int row=0;row<a->rows;row++){
        delete []a->data[row];  //Deletes the datarow by row
    }
    delete []a->data;           //Deletes the array of pointers to the rows
    delete a;                   //Deletes structure
}

void fillAry(Array2D *a, Array1D *c,int wchCol){
    if(a->rows!=c->size){
        cout<<"Not compatible Dimemsions"<<endl;
        exit(1);
    }
    for(int row=0;row<a->rows;row++){
...
Array2D *fillAry(int rows,int cols){
    ...
    Array2D *a=new Array2D;     //Allocate structure
    a->rows=rows<2?2:rows;
    a->cols=cols<2?2:cols;
    a->data=new int*[a->rows];     //Allocating an array of pointers to hold each
                                //data array
    for(int row=0;row<a->rows;row++){
        a->data[row]=new int[a->cols];
    }
    //Fill it with 0's
    for(int row=0;row<a->rows;row++){
